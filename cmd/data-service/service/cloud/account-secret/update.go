...
func batchUpdateForTCloud(svc *accountSecretSvc, cts *rest.Contexts) (interface{}, error) {
	...
	for _, one := range req.AccountSecrets {
		...
		if one.Extension != nil {
			...
			// 查询原有数据以合并 extension
			dbModel, err := getAccountSecretFromTable(one.ID, svc, cts)
			if err != nil {
				return nil, err
			}
...
func getAccountSecretFromTable(id string, svc *accountSecretSvc, cts *rest.Contexts) (
	*tableas.Table, error) {
	...
	listResult, err := svc.dao.AccountSecret().List(cts.Kit, opt)
	if err != nil {
